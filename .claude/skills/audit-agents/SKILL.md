...
- [ ] Quality gate pipeline: `cargo build && cargo test && cargo fmt --check && cargo clippy -- -D warnings`.
- [ ] `.claude/settings.local.json` includes permissions for `cargo fmt` and `cargo clippy`.
