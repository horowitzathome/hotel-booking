...
description: >-
  ...
  for Rust projects. Load when conducting documentation reviews.
compatibility:
  - claude-code
  - opencode
  - github-copilot
...
- No Rust code blocks in PRD
- No Rust-specific constructs in PRD (derive macros, trait bounds, lifetimes, async/await patterns)
...
- Configuration properties, struct fields, and tech stack versions match between documents and source code

### 3. Project-Specific Coherence

- Configuration structs in `docs/system-design.md` match actual config structs in `src/`
- Module structure in `docs/system-design.md` matches actual `src/` directory layout
- Struct definitions referenced in `docs/system-design.md` exist in source code
...
6. For coherence checks, verify config structs and type definitions match between documents and source code.
