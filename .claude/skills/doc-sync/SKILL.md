...
1. Read all Rust source files under `src/` — note every module, struct, enum, trait, function
2. Read config files (e.g., `config.toml`, `.env.example`) for configuration properties
3. Read all test files under `src/` (`#[cfg(test)] mod tests`) and `tests/` directory
4. Read all ADR files under `docs/adr/`
5. Read `Cargo.toml` for dependencies and feature flags

Capture: struct names (exact casing), fields (exact types), trait implementations, module paths, public vs private visibility, pipeline step ordering, CLI arguments, configuration properties, error variants.
...
**In System Design:**
- Struct names that changed (case matters)
- Fields that were added, removed, or retyped
- Module structure changes (new modules, moved modules, visibility changes)
...
- PRD = *what* the system does. No Rust code, struct names, function names, derive macros, or implementation constructs.
- System design = *how* it is built. Struct definitions, module structure, pipeline, error handling. No verbatim service implementations.
...
> You are a read-only reviewer. Inspect files with Read, Glob, and Grep. Only permitted Bash commands: `cargo build`, `cargo test`. Do not write code, scripts, or temporary files. Never use system `/tmp`; use `.scratch/tmp/` for any temporary output.

The reviewer validates against `docs/documentation.md` checklist:
1. Structural checks (cross-references, tables, code blocks)
2. Cross-document coherence (requirement IDs, config properties, struct fields, tech stack versions, module names)
3. Writing standards (prohibited words, sentence length, acronyms)
4. Document boundaries (PRD has no Rust; system-design has no copied source)
