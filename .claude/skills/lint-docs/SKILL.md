...
- **No verbatim source code.** Service implementations, build scripts, and test bodies copied from source files are violations. Domain struct definitions are allowed — they are the design contract.
- **Design-level descriptions only.** Sections should use contracts, algorithms, schemas, patterns, and tables — not runnable code.
- **Compare against source.** For each Rust code block in system-design.md, check if a corresponding source file exists in `src/`. If it does, the code block is a duplication that will go stale.

### 2. PRD Boundary (prd.md)

Read `docs/prd.md` and check:

- **No Rust code blocks** (` ```rust `).
- **No implementation language.** No struct names, function names, derive macros, trait bounds, lifetimes, async patterns.
...
- **Permitted cargo commands.** Reviewer agents must permit only `cargo build`, `cargo test`, `cargo fmt --check`, and `cargo clippy -- -D warnings`.
...
- Configuration properties in prd.md match config structs in source.
