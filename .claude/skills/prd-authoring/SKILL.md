...
**Litmus test:** If it would change when switching from Rust to another language, it belongs in `docs/system-design.md`, not the PRD.
...
|---|---|---|
| Rust code blocks (` ```rust `) | Critical | Move to system-design.md, link from PRD |
| Rust-specific constructs (derive macros, trait bounds, lifetimes, async/await, tokio patterns) | Critical | Describe behavior, not mechanism |
| Internal code references (struct names, function names, variable names) | High | Use behavioral language |
