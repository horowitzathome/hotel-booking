...
pub async fn create(state: web::Data<AppState>, body: web::Json<CreatePersonRequest>) -> Result<HttpResponse, AppError> {
    body.validate()?;
    let person = svc::create(&state.pool, &body).await?;
    let location = format!("/api/v1/persons/{}", person.id);
    Ok(HttpResponse::Created().insert_header(("Location", location)).json(person))
}
...
pub async fn update(state: web::Data<AppState>, path: web::Path<i64>, body: web::Json<UpdatePersonRequest>) -> Result<HttpResponse, AppError> {
    body.validate()?;
