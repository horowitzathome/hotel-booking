...
- **docs/system-design.md:** The module structure diagram and overview paragraph are Level 1. Section headings for types, traits, and modules are Level 2. Per-type and per-function detail blocks are Level 3. Implementation order tables are Level 4.
...
- Rust struct definitions (→ system-design.md)
- Rust-specific constructs: derive macros, trait bounds, lifetimes, async patterns (→ system-design.md)
- Internal code references: struct names, function names, variable names (→ system-design.md)
...
**Owns:**
- Rust conventions for this project (prescriptive style examples)
- Architectural invariants and guardrails
- Constants reference table (modules, source files, key values)
- Module structure (file paths)
- Domain model (Rust structs and enums, single source of truth for types)
- Trait and type summaries with source file pointers
...
- Trait definitions (→ source code is the source of truth)
...
Struct carrying every parameter the `OrderService::process()` function needs: retry policy, timeout, and injected dependencies for testability. See `src/infrastructure/config.rs`.

**Implements:** REQ-OR-001, REQ-OR-003
```
...
See `src/domain/order.rs` for the type definition.
```
...
- Reference system-design.md for types and traits
- Check ADRs for design constraints before proposing alternatives
- Never duplicate type definitions across documents
- Never add pseudocode or Rust code to PRD (use contracts only)
- Never add Rust-specific constructs to PRD (derive macros, trait bounds, lifetimes, async patterns)
- Never reference internal code in PRD (no struct names, function names, or variable names)
- Use behavioral language in PRD ("the system retries the operation" not "`retry_loop()` calls `continue`")
...
├── Has: Module structure (file paths)
...
| Rust code blocks in PRD | **Critical** | Move to system-design.md, link from PRD |
| Rust-specific constructs in PRD (derive macros, trait bounds, lifetimes, tokio patterns) | **High** | Describe behavior in PRD, mechanism in system-design.md |
| Internal code references in PRD (struct names, function names, variable names) | **High** | Use behavioral language |
...
| Exhaustive rule listings in system-design.md (TOML, YAML, shell) | **Medium** | State the invariant; source is authoritative for the full listing |
...
| Language tags on code blocks | Enables syntax detection | ` ```rust` not ` ``` ` |
...
[Constants](../system-design.md#constants) — `DEFAULT_TIMEOUT`, `MAX_RETRIES`
...
**Behavior:**
[Prose description. No code, no Rust constructs, no internal function names. Describe what happens, not how.]
...
- Constraint name: `CONSTANT_NAME` (value)
...
- [ ] No Rust code blocks (` ```rust `) in PRD
- [ ] No Rust-specific constructs in PRD (derive macros, trait bounds, lifetimes, tokio patterns)
...
- [ ] Code blocks have language tags (` ```rust ` not ` ``` `)
...
- [ ] No exhaustive rule listings (TOML, YAML, shell). State the invariant; source is authoritative for the full listing.
...
- [ ] Configuration properties in prd.md match config structs in system-design.md.
